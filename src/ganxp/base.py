...
from utils.bench import PerceptualLoss
...
class Discriminator(nn.Module):
    ...
    def __init__(self, blocks: int | Iterable[int], **kwargs):
        ...
        :keyword n: Block size of each sampling layer
        :keyword p: Dropout probability
        :keyword m_block: multiplier for blocks.
        """
        n = kwargs.pop("n", 1)
        p = kwargs.pop("p", 0)
        ...
        if isinstance(blocks, int):
            ...
            assert isinstance(blocks, Iterable) and "m_block" not in kwargs, \
                f"parameter 'm_block' must be only used with integer valued parameter blocks"
        blocks = list(blocks)
...
class Gan(nn.Module, metaclass=ABCMeta):
    ...
    def loss_step(
            ...
            optim_dis: "optim.Optimizer",
            noise: "torch.Tensor",
            real: "torch.Tensor",
            perceptual: "PerceptualLoss" = None,
            perceptual_lambda: float = 1,
    ) -> dict[str, float]:
        raise NotImplementedError
...
class KLGan(Gan, metaclass=ABCMeta):
    ...
    def loss_step(
        ...
        # perceptual loss
        loss_perceptual = perceptual(fake, real) if perceptual else torch.tensor(0.0)
        # total loss
        loss_G = loss_adversarialG + perceptual_lambda * loss_perceptual
        # backprop
        ...
        metrics = {
            ...
            "loss/adversarial_dis": loss_adversarialD.item(),
            "loss/adversarial_gen": loss_adversarialG.item(),
